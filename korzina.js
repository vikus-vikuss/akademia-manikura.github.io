...
function updateSelectedCoursesQuantity(changeAmount, sumElement) {
    const courseName = sumElement.closest('.modal-content').querySelector('.kurs-name').innerText;
    ...
    if (course) {
        ...
        const cartItems = document.getElementById('cartItems');
        const cartItem = Array.from(cartItems.children).find(item => item.querySelector('.course-name').innerText === course.name);
        
        if (cartItem) {
            cartItem.querySelector('.course-quantity').innerText = `Количество: ${course.quantity}`;
        }
    }
}
